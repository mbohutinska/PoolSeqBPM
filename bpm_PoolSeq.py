...
def calcBPM(input_file, output, outname, window_size, minimum_snps, num_pops):
    ...
    def NestedAnova(locus_list):
        ...
        Nnei = ((af[0]-af[1])**2)/2 # to make maximum 1 instead of 2 (Bhatia, 2013)
        Dnei = 2 * p_avr * (1-p_avr) # to make maximum 1 instead of 2 (Bhatia, 2013)
        afdif = af[0]-af[1]
      #  print(ac_m[0]/an_m[0],ac_m[1]/an_m[1], p_avr,Nnei,Dnei) ###
		...
		#
        if all(p == 0.0 for p in af) or all(p == 1.0 for p in af):
            poly = False
        else:
            poly = True

        return Nnei, Dnei, afdif, poly
    ...
    out1 = open(outfile, 'w')
    out1.write("outname\tscaff\tstart\tend\twin_size\tnum_snps\tAFD\tFixedDiff\tFstN\n")
        ...
        for i, line in enumerate(data):
            ...
            if pos > start and pos <= end and scaff == oldscaff:
                if pos == old_pos:  # Accruing information from multiple populations but same locus
                    ...
                    Nnei, Dnei, afdif, poly = NestedAnova(Locus)
                    if poly is True:
                        #if num_pops == 2:
                            #d, a = calcDxy(Locus)
                            #dxy += d
                ...
                if snp_count >= minimum_snps:  # Report or exclude window
                    ...
                    try:
                        ...
                        print('ZeroDivisionError:', snp_count, fstN, FstN)
...
if __name__ == '__main__':  # Used to run code from command line
    ...
    j1, j2 = calcBPM(args.i, args.o, args.prefix, args.ws, args.ms, args.np)
